...
void Misn18Mission::Execute(void)
{
	...
	*/
		...
		(missionstart == false)
	{
		...
		SetObjectiveName(transport, "Fury Transport");
	...
	if (transportfound == false)
		{
			if (GetTime()>next_second)
			{
				if
					(IsAlive(thrusterone))
				{
					AddHealth(thrusterone, 50.0f);
				}
				if
					(IsAlive(thrustertwo))
				{
					AddHealth(thrustertwo, 50.0f);
				}
				if
					(IsAlive(thrusterthree))
				{
					AddHealth(thrusterthree, 50.0f);
				}
				if
					(IsAlive(thrusterfour))
				{
					AddHealth(thrusterfour, 50.0f);
				}
...
const std::set<std::string>& Misn18Mission::GetPreloadSet()
{
