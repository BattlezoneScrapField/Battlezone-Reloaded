...
private:
	...
	EObjectiveColor obj1, obj2, obj3;
...
void Misns1Mission::AddObject(BZHandle h)
{
}

static BZCOLOR getColorFromObj(EObjectiveColor e)
{
	if (e == EObjectiveColor::WHITE_OBJ)
		return WHITE;
	if (e == EObjectiveColor::GREEN_OBJ)
		return GREEN;
	if (e == EObjectiveColor::RED_OBJ)
		return RED;
...
void Misns1Mission::Execute(void)
{
	...
	*/
		...
		(missionstart == false)
	{
		...
		SetObjectiveName(walkcam1, "Walker Cut Off");
		//SetObjectiveName(walkcam2, "Middle Pass Exit");
		SetObjectiveName(ambase, "American Outpost");
		SetObjectiveName(hidcam1, "Upper Pass Exit");
		SetObjectiveName(hidcam2, "Middle Pass Exit");
		SetObjectiveName(hidcam3, "Lower Pass Exit");
		SetObjectiveName(basecam, "Home Base");
	...
	{
		EObjectiveColor o1, o2, o3 = EObjectiveColor::UNKNOWN_OBJ;

		if ((IsAlive(colorado)) && (coloradosafe == false))
		{
			o1 = EObjectiveColor::WHITE_OBJ;
		}

		if ((!IsAlive(colorado)) && (coloradosafe == false))
		{
			o1 = EObjectiveColor::GREEN_OBJ;
		}

		if (coloradoreachedsafepoint == true)
		{
			o1 = EObjectiveColor::RED_OBJ;
		}

		if (coloradosafe == false)
		{
			if ((IsAlive(muf)) || (IsAlive(silo)))
			{
				o2 = EObjectiveColor::WHITE_OBJ;
			}
		}

		if (coloradosafe == true)
		{
			if ((IsAlive(muf)) || (IsAlive(silo)) || (IsAlive(colorado)))
			{
				o2 = EObjectiveColor::WHITE_OBJ;
			}
		}
		...
		if ((IsAlive(svrec)) && (missionwon == false))
		{
			o3 = EObjectiveColor::WHITE_OBJ;
		}

		if (!IsAlive(svrec))
		{
			o3 = EObjectiveColor::RED_OBJ;
		}

		if (missionwon == true)
		{
			o3 = EObjectiveColor::GREEN_OBJ;
		}

		if ((coloradosafe == true) && (missionwon == false))
		{
			o1 = EObjectiveColor::RED_OBJ;
		}
		...
		if (obj1 != o1 || obj2 != o2 || obj3 != o3)
		{
			...
			if (obj1 != EObjectiveColor::UNKNOWN_OBJ)
				AddObjective("misns101.otf", getColorFromObj(obj1));

			if (obj2 != EObjectiveColor::UNKNOWN_OBJ)
				AddObjective("misns102.otf", getColorFromObj(obj2));

			if (obj3 != EObjectiveColor::UNKNOWN_OBJ)
				AddObjective("misns103.otf", getColorFromObj(obj3));
		}
	}
			

	if((pickpath == false) && (startconvoy < GetTime ()))
	{
		switch (path)
		{
		case 0:
...
{
	obj1 = obj2 = obj3 = EObjectiveColor::UNKNOWN_OBJ;
}
...
const std::set<std::string>& Misns1Mission::GetPreloadSet()
{
