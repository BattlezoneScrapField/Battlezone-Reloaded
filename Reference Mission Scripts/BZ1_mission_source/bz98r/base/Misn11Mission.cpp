...
void Misn11Mission::Execute(void)
{
	...
	if (!start_done)
	{
		...
		*/
		...
		SetObjectiveName(cam1, "Waypoint 1");
		SetObjectiveName(cam2, "Waypoint 2");
		SetObjectiveName(cam3, "Launch Pad");
		SetObjectiveOn(tug1); 
		SetObjectiveName(tug1, "Transport 1"); 
		Stop(tug1, 1);
		SetCritical(tug1, true);
		SetObjectiveOn(tug2); 
		SetObjectiveName(tug2, "Transport 2"); 
		Stop(tug2, 1);
		SetCritical(tug2, true);
		SetObjectiveOn(openh);
		SetObjectiveName(openh, "Transport 3"); 
		Stop(openh, 1);
		SetCritical(openh, true);
	...
	*/
	if (IsAlive(openh))AddHealth(openh, 300.0f);
	...
	if (Get_Time()>start_delay)
	{
		...
		*/
		...
		Goto(tug1,"base1",1);
		Goto(tug2,"base1",1);
		Goto(openh,"base1",1);	// was originally 0
	}
	/*
	...
	*/
	...
	if ((!last_wave) && (last_wave_time<GetTime()))
	{
		...
		BZHandle last_guy=BuildObject("svfigh",2,launch2);
...
const std::set<std::string>& Misn11Mission::GetPreloadSet()
{
