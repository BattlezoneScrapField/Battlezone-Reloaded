	...
	union {
		struct {
			float
				...
				Checkdist, Checkdist2, Checkalive,
				f_last;
		};
		float f_array[7];
	};
...
void Misns3Mission::AddObject(BZHandle h)
{
}
...
void Misns3Mission::Execute(void)
{
	...
	*/
		...
		(missionstart == false)
	{
		...
		SetObjectiveName(cam1, "Home Base");
		SetObjectiveName(cam2, "Black Dog Outpost");
	}
	player = GetPlayerHandle();
...
const std::set<std::string>& Misns3Mission::GetPreloadSet()
{
