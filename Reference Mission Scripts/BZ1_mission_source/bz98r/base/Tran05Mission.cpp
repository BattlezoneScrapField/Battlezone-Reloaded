...
void Tran05Mission::AddObject(BZHandle h)
{
	if (
		(GetTeamNum(h) == 1) &&
		(IsOdf(h, "avscav"))
		&& (bscav==BZINVALIDHANDLE)
		)
	{
		found = true;
		bscav=h;
...
void Tran05Mission::Execute(void)
{
	...
	{	
		if (CameraPath("zoomcam",1200,800,dummy) 
			|| GetTime() > cam_time /*IsAudioMessageDone(audmsg)*/
			|| CameraCancelled() )
		{
			camera3=false;
			cam_time=99999.0f;
...
//		if (bscout!=NULL) Attack(bscout,bscav,1);
	...
	if ((message1) && (message5) && (!message2) &&
		(
			(GameObjectHandle::GetObj(bscav) == NULL) ||
			(GameObjectHandle::GetObj(bscav)->GetLastEnemyShot()>0)
		))
	{
		if (GameObjectHandle::GetObj(bscav) != NULL)
		{
			// send the scav home
			// bscav to bbase
			Follow(bscav, bhome, 0);
		}
		...
		*/
		...
		BZHandle bbase=GetHandle("apbase-1_camerapod");
		SetUserTarget(bbase);
		message2=true;
	}
	if (((bscav!=BZINVALIDHANDLE) &&  // was message2, so we know a scav was built
		((!IsAlive(bplayer)) || 
		(!IsAlive(bscav)) || 
	...
	{
		AddHealth(bscav, 200.0f);
		NextSecond=GetTime()+1.0f;
	}
	if (last_wave_time<GetTime())
	{
		BZHandle sid=BuildObject("svfigh",2,"spawn4");
		Attack(sid,scav2);
		last_wave_time=99999.0f;
	}
	...
	(message3)	&& (!mission_won))
	{
		...
		AddHealth(bscav, 1000.0f);
		AddHealth(scav2, 1000.0f);
...
const std::set<std::string>& Tran05Mission::GetPreloadSet()
{
