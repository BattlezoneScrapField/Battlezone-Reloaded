	...
	union {
		struct {
			int32_t
				...
				investigator,
				warn,
				aud1, aud2, aud3, aud4, aud10, aud11, aud12, aud13, aud14, aud20, aud21, aud22, aud23,
				i_last;
		};
		int32_t i_array[18];
	};
...
void Misn04Mission::AddObject(BZHandle h)
{
	if (
		(GetTeamNum(h) == 1) &&
		(IsOdf(h, "avhaul"))
...
every frame.  
	*/
	...
	if
		(IsAlive(cam1))
	{
		AddHealth(cam1, 100.0f);
	}
	if
		(IsAlive(cam2))
	{
		AddHealth(cam2, 100.0f);
	}
	if
		(IsAlive(cam3))
	{
		AddHealth(cam3, 100.0f);
	}
...
const std::set<std::string>& Misn04Mission::GetPreloadSet()
{
