...
class Misn06Mission : public AiMission {
	DECLARE_RTIME(Misn06Mission)
public:
	Misn06Mission(void);
	~Misn06Mission();
...
void Misn06Mission::AddObject(BZHandle h)
{
}
...
void Misn06Mission::Execute(void)
{
	...
	*/
...

	{
		...
		VECTOR_3D h_pos = GetPosition(haephestus);
		h_pos.y -= 4;
		SetPosition(haephestus, h_pos);					//KLUDGE TO FIX COLLISION ISSUE WITH HEPHESTUS REAR 
	...
	if
		(IsAlive(star2))
	{
		AddHealth(star2, 10000.0f);
	}
	if
		(IsAlive(star6))
	{
		AddHealth(star6, 10000.0f);
	}
	if
		(IsAlive(star8))
	{
		AddHealth(star8, 10000.0f);
	}
	if(IsAlive(haephestus))
	   {
		   AddHealth(haephestus, 10000.0f);
	   }
		...
		(opencamdone == true)
	{
		...
		AddHealth(p5u3, 50.0f);
		AddHealth(p5u4, 50.0f);
		AddHealth(p5u6, 50.0f);
		AddHealth(p5u9, 50.0f);
		AddHealth(p5u12, 50.0f);
	}

	if
		(
		(opencamdone == true) && ((opencamtime < GetTime() && IsAudioMessageDone(audmsg)) || CameraCancelled())
		)
	{
	...
	if
		(
		(haephestusdisc == true) && (reconheaphestus == false) && (hephikey == false)
		&& (hephdisctime < GetTime())
		)
	{
		if (hephwarn < 2)
		{
			aud105 = AudioMessage("misn0690.wav");
			hephdisctime = GetTime() + 20.0f;
			hephwarn = hephwarn + 1;
		}
		else if (missionfail4 == false)
		{
		 ...
		 )
	{
		if (ident < 2)
		{
			aud100 = AudioMessage("misn0691.wav");
			ident = ident + 1;
			identtime = GetTime() + 10.0f;
		}
		else if (missionfail == false)
		{
	...
	*/
	...
	if ( (!IsAlive(player)) && (IsOdf(player, "asuser")) )
	{
		FailMission(Get_Time() + 5.0f);
	}
}	

IMPLEMENT_RTIME(Misn06Mission)
...
void Misn06Mission::AddObject(GameObject *gameObj)
{
	AddObject(gameObj->GetHandle());
	AiMission::AddObject(gameObj);
}

bool Misn06Mission::Load(file fp)
{
	if (missionSave) {
		int i;
...
bool Misn06Mission::Save(file fp)
{
	if (missionSave)
...
const std::set<std::string>& Misn06Mission::GetPreloadSet()
{
