...
void BlackDog07Mission::AddObject(BZHandle h)
{
}
...
void BlackDog07Mission::Setup()
{
	...
	BZHandle h = GetHandle("nav_mybase");
...
void BlackDog07Mission::Execute()
{
	...
	if (annoyTime < GetTime())
	{
		...
		for (i = 0; i < 3; i++)
		{
			BZHandle h = BuildObject("cvfigh", 2, "annoy_1");
			Attack(h, user);
		}
		for (i = 0; i < 2; i++)
		{
			BZHandle h = BuildObject("cvltnk", 2, "annoy_1");
			Attack(h, user);
		}
	}
...
const std::set<PrjID>& BlackDog07Mission::GetPreloadGameObjects()
{
