...
class Misn04Mission : public AiMission {
	DECLARE_RTIME(Misn04Mission)
public:
	Misn04Mission(void);
	~Misn04Mission();
	...
	union {
		struct {
			float
				...
				endcindone, aud1, aud2, aud3, aud4, aud10, aud11, aud12, aud13, 
				aud14, aud20, aud21, aud22, aud23, notfound, ccatug, cintime1, 
...
every frame.  
	*/
		...
		(missionstart == false)
	{
		...
		GameObjectHandle :: GetObj(cam1) ->SetName ("SW Geyser");
		GameObjectHandle :: GetObj(cam2) ->SetName ("NW Geyser");
		GameObjectHandle :: GetObj(cam3) ->SetName ("NE Geyser");
		GameObjectHandle :: GetObj(basecam) ->SetName ("CCA Base");
	...
	if
		(IsAlive(cam1))
	{
	GameObjectHandle::GetObj(cam1)->AddHealth(100.0f);
	}
	if
		(IsAlive(cam2))
	{
	GameObjectHandle::GetObj(cam2)->AddHealth(100.0f);
	}
	if
		(IsAlive(cam3))
	{
	GameObjectHandle::GetObj(cam3)->AddHealth(100.0f);
	}
	...
	{
		GameObjectHandle :: GetObj(reliccam) -> SetName("Investigate CCA");
		newobjective = true;
		obset = false;
	}
...
IMPLEMENT_RTIME(Misn04Mission)

Misn04Mission::Misn04Mission(void)
{
}

Misn04Mission::~Misn04Mission()
{
}

void Misn04Mission::AddObject(GameObject *gameObj)
{
	AddObject(gameObj->GetHandle());
	AiMission::AddObject(gameObj);
}

bool Misn04Mission::Load(file fp)
{
	if (missionSave) {
		int i;
...
bool Misn04Mission::PostLoad(void)
{
	if (missionSave)
...
bool Misn04Mission::Save(file fp)
{
	if (missionSave)
...
void Misn04Mission::Update(void)
{
	AiMission::Update();
	Execute();
}
