...
void Chinese08Mission::AddObject(BZHandle h)
{
}
...
void Chinese08Mission::Setup()
{
	...
	west_powerDead = BZFALSE;
	east_powerDead = BZFALSE;
	west_commDead = BZFALSE;
	east_commDead = BZFALSE;
...
*/
		...
		case MS_WAITFORJUMPOUT:
			// Fail on bail (it invalidates a vehicle)
			if(stateTimer < GetTime() || !IsValid(apc[4]))
			{
				missionState = MS_END;
		...
		case MS_WAITFORWRECKER:
			// Fail on apc destroyed, should stick around until bomb hits
			// also, user should never jump back into the apc
			if (apc[4] == user || !IsValid(apc[4]))
			{
				missionState = MS_END;
				FailMission(GetTime() + 2.0f, "ch08lseg.des");
				break;
		...
		case MS_DESTROYPOWERS:
			...
			{
				west_powerDead = BZTRUE;

				if(!east_powerDead && !west_commDead && !east_commDead)
				{
					StartCockpitTimer(long(60 * 4.5f));
				}
...
*/
		...
		case MS_WRECKER106:
			if(stateTimer < GetTime())
			{
				...
				resetObjectives();
				SucceedMission(GetTime() + 13.0f, "ch08win.des");
				// play avi...
			}
			break;
	}
...
const std::set<PrjID>& Chinese08Mission::GetPreloadGameObjects()
{
	static std::set<PrjID> retval = AiMission::PrjIDFromStrings(
	{ "apcamr", "cvrecy", "cvmuf", "cvslf", "svapc", "svfigh" }
	);
	return retval;
}
