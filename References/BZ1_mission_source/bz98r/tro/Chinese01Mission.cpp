...
void Chinese01Mission::AddObject(BZHandle h)
{
}
...
void Chinese01Mission::Setup()
{
	...
	objective4Complete = BZFALSE;
	objective5Complete = BZFALSE;
	hangarIdentified = BZFALSE;
	recyclerDeployed = BZFALSE;
	tugNearNav = BZFALSE;
	arialsSpawned = BZFALSE;
	focusOnExplosion = BZFALSE;
	doingExplosion = BZFALSE;
	doNuke = BZFALSE;
	sound12Played = BZFALSE;
	decoySpawned = BZFALSE;
...
void Chinese01Mission::Execute()
{
	...
	//make the hangar indestuctible

	SetCurHealth(hangar,10000.0f);
	SetMaxHealth(hangar,10000.0f);
	...
	if (hangarSound != BZINVALIDHANDLE && IsAudioMessageDone(hangarSound))
	{
		hangarSound = BZINVALIDHANDLE;
	...
	if (armourySound != BZINVALIDHANDLE && IsAudioMessageDone(armourySound))
	{
		armourySound = BZINVALIDHANDLE;
	...
	if (!objective2Complete && GetHealth(commTower) <= 0.0f)
	{
		...
		hangarIdentified = BZTRUE;    //fixes blowing up the tower early bug.
		...
		AddObjective("ch01006.otf", WHITE);
		AddObjective("ch01007.otf", WHITE);
	}

	if (recycler != BZINVALIDHANDLE && !recyclerDeployed && isDeployed(recycler))
	{
		recyclerDeployed = BZTRUE;

		wave1Time = GetTime() + 60.0f;
		ClearObjectives();
		AddObjective("ch01006.otf", GREEN);
		AddObjective("ch01007.otf", WHITE);
	}
...
//		Goto(h, "follow_4", 1);
	...
	if (tug != BZINVALIDHANDLE && !tugNearNav && GetDistance(tug, "nav_tug", 0) < 1000.0f)
	{
		tugNearNav = BZTRUE;
		...
		// update objectives
		ClearObjectives();
		AddObjective("ch01007.otf", GREEN);
		AddObjective("ch01003.otf", WHITE);
	}

	// has the tug been killed?
	if (tug != BZINVALIDHANDLE && GetHealth(tug) <= 0.0f && !lost && !won)
	{
		lost = BZTRUE;
	...
	if (failedSound != BZINVALIDHANDLE && IsAudioMessageDone(failedSound))
	{
		failedSound = BZINVALIDHANDLE;

		FailMission(GetTime() + 1.0f, "ch01lseb.des");
	}

	// has the tug gotten away from us?
	if (tug != BZINVALIDHANDLE && GetDistance(tug, "tug_fail") < 350.0f && GetTeamNum(tug) == 2 && !lost && !won)
	{
	...
	if (tug != BZINVALIDHANDLE && GetDistance(tug, recycler) < 75.0f && !objective3Complete)
	{
		// message
	...
	{
		bool withinRange = BZFALSE;
		for (i = 0; i < 10; i++)
		{
			if (IsAlive(detectors[i]) && GetDistance(user, detectors[i]) < 350.0f)
			{
				withinRange = BZTRUE;
				break;
			}
		...
		if (!arialsSpawned && withinRange)
		{
			...
			h = BuildObject("sssold", 2, "aerial_4", 100);
			h = BuildObject("sssold", 2, "aerial_4", 100);
			arialsSpawned = BZTRUE;
		}

	
		SetCurAmmo(escort1, 0);
		SetCurAmmo(escort2, 0);
		SetMaxAmmo(escort1, 0);
		SetMaxAmmo(escort2, 0);
...
//	SetPerceivedTeam(escort2, 2);
	...
	// we've been detected
	if (detectedSound != BZINVALIDHANDLE && IsAudioMessageDone(detectedSound))
	{
		detectedSound = BZINVALIDHANDLE;
		//failedSound = AudioMessage("ch01011.wav");
		FailMission(GetTime(), "ch01lsec.des");
	}
	// if we get the relic
	if (objective3Complete && !objective4Complete && 
		relic != BZINVALIDHANDLE && tug != BZINVALIDHANDLE && GetCargo(tug) == relic)
	{
		//Trace("Relic picked up by Tug");
		objective4Complete = BZTRUE;
		...
		assert(navEnd != BZINVALIDHANDLE);
		SetName(navEnd, GetTextLabel("names", "Safe Distance"));
	...
	if (objective4Complete && !decoySpawned)
	{
		...
		if (withinRange)
		{
			...
			SetCurAmmo(escort1, 100);
			SetCurAmmo(escort2, 100);
			SetMaxAmmo(escort1, 100);
			SetMaxAmmo(escort2, 100);

			for (i = 0; i < 7; i++)
			{
				BZHandle h = BuildObject("cvhtnk", 1, "decoy_units");
	...
	if (objective4Complete && !doingExplosion)
	{
		...
		else if (GetCockpitTimer() <= 2.0 && !focusOnExplosion)
		{
			...
			if (GetDistance(user, hangar) > nukeDistance && !focusOnExplosion)
			{
				...
				focusOnExplosion = BZTRUE;
			}
		}

		else if (GetCockpitTimer() <= 5.0 && !objective5Complete)
		{
			// pre-explosion
			float nukeDistance = GetDistance(navEnd, hangar) - 50;
			if (GetDistance(user, hangar) > nukeDistance && !focusOnExplosion)
	...
	if (doNuke)
	{
		...
		{
			SucceedMission(GetTime() + 3.0f, "ch01win.des");
			won = BZTRUE;
		}
		else
		{
			FailMission(GetTime() + 3.0f, "ch01lsee.des");
			lost = BZTRUE;
		}
		//			MakeExplosion("spawn_explosion1", "xpltrsn");   //"spawn_explosion" doesn't exist to hard coding.
		VECTOR_3D explodePos = { 1040, 40, 101970 };
		MakeExplosion(explodePos, "xpltrsn");   //"spawn_explosion" doesn't exist to hard coding.
	}

	// has the recycler been lost?
	if (recycler != BZINVALIDHANDLE && GetHealth(recycler) <= 0.0 && !objective3Complete &&
		!lost && !won)
	{
		lost = BZTRUE;
		FailMission(GetTime() + 1.0f, "ch01lsed.des");
	}
}

const std::set<PrjID>& Chinese01Mission::GetPreloadGameObjects()
{
	static std::set<PrjID> retval = AiMission::PrjIDFromStrings(
	{ "apcamr", "cvslfb", "cvrecyd", "svfigh", "svtank",
	"svltnk", "svhraz", "sspilo", "sssold", "svhaula", "obdata", "svturrb"}
	);
	return retval;
}
