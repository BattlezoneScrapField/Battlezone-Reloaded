...
#define SAFE_DISTANCE_FROM_CONVOY  175.0f //SHOULD BE 175
#define TOO_FAR_FROM_TUG_DISTANCE  700.0f
/*
	Chinese05Mission
*/
...
private:
	...
	union {
		...
		BZHandle h_array[20 + MAX_MUST_KILL];
	};
...
void Chinese05Mission::AddObject(BZHandle h)
{
}
...
void Chinese05Mission::Setup()
{
	...
	mustStayWithin200MetresOfSilo = BZFALSE;
	scoutCloseToSilo = BZFALSE;
	trigger1 = BZFALSE;
	trigger2 = BZFALSE;
	tugSpawned = BZFALSE;
	tugGotScout = BZFALSE;
	closeToScout = BZFALSE;
	wave7Spawned = BZFALSE;
	scoutsSpawned = BZFALSE;
	stayTeam2 = BZFALSE;
	mustBeCloseToTug = BZFALSE;
...
BZHandle Chinese05Mission::getBase()
{
	BZHandle h[3];
	int numH = 0;
...
void Chinese05Mission::Execute()
{
	...
	if (!startDone)
	{
		...
		openingSoundTime = GetTime() + 3.0f;
		
		mustStayWithin200MetresOfSilo = BZTRUE;
		ClearObjectives();
		...
		!tugGotScout)
	{
		...
		mustBeCloseToTug = BZTRUE;
	}

	// too close to the tug stuff?
	if (tugSpawned && 
		stayTeam2 &&
		GetDistance(user, tug) < SAFE_DISTANCE_FROM_CONVOY &&
		!lost && !won)
	{
		lost = BZTRUE;
		FailMission(GetTime() + 1.0f, "ch05lseb.des");
	}

	if (mustBeCloseToTug &&
		GetDistance(user, tug) > TOO_FAR_FROM_TUG_DISTANCE &&
		!lost && !won)
	{
		lost = BZTRUE;
		FailMission(GetTime() + 1.0f, "ch05lsed.des");
	}
	...
	if (betty6 != BZINVALIDHANDLE && IsAudioMessageDone(betty6))
	{
		betty6 = BZINVALIDHANDLE;
	...
	if (betty14 != BZINVALIDHANDLE && IsAudioMessageDone(betty14))
	{
		betty14 = BZINVALIDHANDLE;
	...
	if (annoy2Time < GetTime())
	{
		...
		for (i = 0; i < 8; i++)
		{
			BZHandle h = BuildObject("sssold", 2, "aerial_1", 400);
			Attack(h, getBase());
		}
	}
	...
	if (navBase != BZINVALIDHANDLE && GetDistance(navBase, user) < 100.0f)
	{
		SetObjectiveOff(navBase);
		navBase = BZINVALIDHANDLE;
	}
	...
	if (wave7Time < GetTime())
	{
		...
		Goto(mustKill[numMustKill++], "wave_7_path");
		assert(numMustKill <= MAX_MUST_KILL);
	...
	if (artl1 != BZINVALIDHANDLE && artl2 != BZINVALIDHANDLE && 
		GetHealth(artl1) <= 0.0 && GetHealth(artl2) <= 0.0)
	{
		artl1 = BZINVALIDHANDLE;
		artl2 = BZINVALIDHANDLE;
		day1Time = 999999.9f;
		day2Time = 999999.9f;
		day3Time = 999999.9f;
		day4Time = 999999.9f;
	...
	if (annoy1Time < GetTime())
	{
		...
		const char *paths[2] = { "annoy_path_1", "annoy_path_2" };
		const char *p = paths[rand() % 2];
		BZHandle h;
	...
	if (recycler != BZINVALIDHANDLE && factory != BZINVALIDHANDLE &&
		GetHealth(recycler) <= 0.0f && GetHealth(factory) <= 0.0f &&
		!lost && !won)
	{
		lost = BZTRUE;
		FailMission(GetTime() + 1.0f, "ch05lsec.des");
	}
	...
	if (wave7Spawned && !won && !lost)
	{
		...
		if (won)
		{
			SucceedMission(GetTime() + 1.0f, "ch05win.des");
		}
	}
}

const std::set<PrjID>& Chinese05Mission::GetPreloadGameObjects()
{
	static std::set<PrjID> retval = AiMission::PrjIDFromStrings(
	{ "apcamr", "apwrck", "cvcnst", "cvfigh", "cvfighh", "cvmuf",
	"sssold", "ssuser", "svartl", "svfigh", "svhraz", "svltnk"
	"svrckt", "svtank", "svwalk" }
	);
	return retval;
}
