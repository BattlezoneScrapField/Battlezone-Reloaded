...
void BlackDog13Mission::AddObject(BZHandle h)
{
}
...
void BlackDog13Mission::Setup()
{
	...
	{
		recycled[i] = BZFALSE;
		defendersSpawned[i] = BZFALSE;
	}
	silosRecycled = BZFALSE;
	sound2Played = BZFALSE;
	arrived = BZFALSE;
	recycleChecked = BZFALSE;
...
void BlackDog13Mission::Execute()
{
	...
	{
		wave1Time = 999999.9f;
		const char *units[7] = { "cvfigh", "cvfigh", "cvfigh", "cvfigh", "cvltnk", "cvltnk", "cvtnk" };

		for (i = 0; i < 7; i++)
		{
			BZHandle h = BuildObject(units[i], 2, "spawn_attack_waves");
			Goto(h, recycler, 1);
		}
	}
	...
	{
		wave2Time = 999999.9f;
		const char *units[6] = { "cvrckt", "cvrckt", "cvltnk", "cvltnk", "cvtnk", "cvtnk" };
	...
	if (wave3Time < GetTime())
	{
		...
		const char *units[9] = { "cvhraz", "cvhraz", "cvhraz",
	...
	if (wave4Time < GetTime())
	{
		...
		const char *units[6] = { "cvfigh", "cvfigh", "cvfigh", "cvfigh", "cvhtnk", "cvhtnk" };
	...
	if (sound5 != BZINVALIDHANDLE && IsAudioMessageDone(sound5))
	{
		...
		FailMission(GetTime() + 1.0f, "bd13lsea.des");
	}
	...
	if (!silosRecycled && !lost && !won)
	{
		...
		for (i = 0; i < 6; i++)
		{
			...
			if (GetHealth(silos[i]) <= 0.0)
			{
				...
				{
					lost = BZTRUE;
					FailMission(GetTime() + 1.0f, "bd13lsec.des");
				}
			}
		}

		// check to see if they're all recycled
		silosRecycled = BZTRUE;
		for (i = 0; i < 6; i++)
		{
			if (!recycled[i])
				silosRecycled = BZFALSE;
	...
	if (sound4 != BZINVALIDHANDLE && IsAudioMessageDone(sound4))
	{
		...
		FailMission(GetTime() + 1.0f, "bd13lseb.des");
	}
	...
	{
		lost = BZTRUE;
		FailMission(GetTime() + 1.0f, "bd13lsed.des");
	}
	...
	if (sound3 != BZINVALIDHANDLE && IsAudioMessageDone(sound3))
	{
		...
		SucceedMission(GetTime() + 1.0f, "bd13win.des");
	}
}

const std::set<PrjID>& BlackDog13Mission::GetPreloadGameObjects()
{
	static std::set<PrjID> retval = AiMission::PrjIDFromStrings(
	{ "cvfigh", "cvltnk", "cvtnk", "cvrckt", "cvhraz", "cvhtnk", "cvrckt" }
	);
	return retval;
}
