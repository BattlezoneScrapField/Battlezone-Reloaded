...
void Misn13Mission::AddObject(BZHandle h)
{
	if ((sv1 == BZINVALIDHANDLE) && (IsOdf(h,"svapc13")))
	{
		sv1 = h;
	}
	else if ((sv2 == BZINVALIDHANDLE) && (IsOdf(h,"svapc13")))
	{
		sv2 = h;
	}		
	else if ((sv3 == BZINVALIDHANDLE) && (IsOdf(h,"svhr13")))
	{
		sv3 = h;
	}
	else if ((sv4 == BZINVALIDHANDLE) && (IsOdf(h,"svhr13")))
	{
		sv4 = h;
	}
	...
	else if ((controltower == BZINVALIDHANDLE) && (IsOdf(h,"abcomm")))
	{
		controltower = h;
	}
	else if ((tank5 == BZINVALIDHANDLE) && (IsOdf(h,"svtk13")))
	{
		tank5 = h;
	}
	else if ((tank6 == BZINVALIDHANDLE) && (IsOdf(h,"svtk13")))
	{
		tank6 = h;
	}
	else if ((tank7 == BZINVALIDHANDLE) && (IsOdf(h,"svtk13")))
	{
		tank7 = h;
	}
	else if ((tank8 == BZINVALIDHANDLE) && (IsOdf(h,"svtk13")))
	{
		tank8 = h;
	}
	else if ((nsdfmuf == BZINVALIDHANDLE) && (IsOdf(h,"avmuf")))
	{
		nsdfmuf = h;
	}
	else if ((nsdfrig == BZINVALIDHANDLE) && (IsOdf(h,"avcnst")))
	{
		nsdfrig = h;
	}
	else if ((avscav1 == BZINVALIDHANDLE) && (IsOdf(h,"avscav")))
	{
		avscav1 = h;
	}
	else if ((avscav2 == BZINVALIDHANDLE) && (IsOdf(h,"avscav")))
	{
		avscav2 = h;
	}
	else if ((avscav3 == BZINVALIDHANDLE) && (IsOdf(h,"avscav")))
	{
		avscav3 = h;
	}
}
...
//		Defend(ccaapc);
		...
		SetObjectiveName(nav1, "Drop Zone");
		Defend(escort_tank);
...
// this is what happens if the silos get attacked
	...
	if ((IsAlive(ccasilo1)) && (!silos_attacked) && (GetHealth(ccasilo1) < 0.95f))
	{
		new_orders_time = Get_Time() + 2.0f;
		silos_attacked = true;
	}
	if ((IsAlive(ccasilo2)) && (!silos_attacked) && (GetHealth(ccasilo2) < 0.95f))
	{
		new_orders_time = Get_Time() + 2.0f;
		silos_attacked = true;
	}
	if ((IsAlive(ccasilo3)) && (!silos_attacked) && (GetHealth(ccasilo3) < 0.95f))
	{
		new_orders_time = Get_Time() + 2.0f;
		silos_attacked = true;
	}
	if ((IsAlive(ccasilo4)) && (!silos_attacked) && (GetHealth(ccasilo4) < 0.95f))
	{
		new_orders_time = Get_Time() + 2.0f;
		silos_attacked = true;
	}
...
// this is what happens if the muf is under attack
	...
	if ((IsAlive(ccamuf)) && (!muf_attacked) 
		&& (GetHealth(ccamuf) < 0.90f) && (!muf_safe))
	{
		if (IsAlive(turret1))
		{
			Goto(turret1, ccamuf);
...
// this is the section that moves the first wave of soviet artillery units
	...
	if ((artil_move) && (artil_move_time < Get_Time()) && (!artil_move2))
	{
		...
		if (GetDistance(artil4, split_geyser) < 20.0f)
		{
			...
			artil_set_time = Get_Time() + 120.0f;
			artil_move2 = true;
		}
	}
	...
	if ((artil_move2) && (!artil_message))
	{
		...
		{
			shot_by = GetWhoShotMe(nsdfrecycle);
...
// win/loose conditions
	...
	if ((!IsAlive(ccamuf)) && (!game_over))
	{
		...
		ClearObjectives();
		AddObjective("misn1300.otf", GREEN);
...
const std::set<std::string>& Misn13Mission::GetPreloadSet()
{
