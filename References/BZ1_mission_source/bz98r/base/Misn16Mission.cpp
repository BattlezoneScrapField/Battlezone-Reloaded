...
void Misn16Mission::AddObject(BZHandle h)
{
	/*
...
void Misn16Mission::Execute(void)
{
	...
	if (!start_done)
	{
		...
		if (cam1!=BZINVALIDHANDLE) SetObjectiveName(cam1,"NW Geyser");
		if (cam2!=BZINVALIDHANDLE) SetObjectiveName(cam2,"Foothill Geysers");
		if (cam3!=BZINVALIDHANDLE) SetObjectiveName(cam3,"Geyser Site");
		if (cam4!=BZINVALIDHANDLE) SetObjectiveName(cam4,"Alien HQ");
		...
		if (sat1!=BZINVALIDHANDLE) Defend(sat1,1);
		if (sat2!=BZINVALIDHANDLE) Defend(sat2,1);
		if (sat3!=BZINVALIDHANDLE) Defend(sat3,1);
		BZHandle sav1 = GetHandle("hvsav47_sav");
		if (sav1!=BZINVALIDHANDLE) Defend(sav1, 1);
		...
		(GetTime()>cam_time1) || IsAudioMessageDone(audmsg2)))
	{
		...
		// release the sav
		BZHandle sav1 = GetHandle("hvsav47_sav");
		if (sav1 != BZINVALIDHANDLE) Stop(sav1, 0);
	}
	...
	if (GetTime()>start_time)
	{
		...
		{
			BZHandle enemy=GetNearestEnemy(player);
	...
	{
		BZHandle sat=BuildObject("hvsat",2,"sat1");
		Goto(sat,"strike1");
	...
	*/
		...
		)
	{
		/*
			...
			Counter attack!!
		*/
		BZHandle sav1=BuildObject("hvsav",2,base2);
		BZHandle sav2=BuildObject("hvsav",2,base2);
//		BZHandle sav3=BuildObject("hvsav",2,base2);
		Attack(sav1,muf,1);
		Attack(sav2,muf,1);
//		Attack(sav3,muf,1);
		counter=true;
...
const std::set<std::string>& Misn16Mission::GetPreloadSet()
{
