...
void Misn09Mission::AddObject(BZHandle h)
{
	if ((cca1 == BZINVALIDHANDLE) && (IsOdf(h,"svturr")))
	{
		cca1 = h;
	}
	else
	{
		if ((cca2 == BZINVALIDHANDLE) && (IsOdf(h,"svturr")))
		{
			cca2 = h;
		}
		else
		{
			if ((cca3 == BZINVALIDHANDLE) && (IsOdf(h,"svturr")))
			{
				cca3 = h;
			}
			else
			{
				if ((cca4 == BZINVALIDHANDLE) && (IsOdf(h,"svturr")))
				{
					cca4 = h;
				}
				else
				{
					if ((cca5 == BZINVALIDHANDLE) && (IsOdf(h,"svfigh")))
					{
						cca5 = h;
					}
					else
					{
						if ((cca6 == BZINVALIDHANDLE) && (IsOdf(h,"svfigh")))
						{
							cca6 = h;
						}
						else
						{
							if ((cca7 == BZINVALIDHANDLE) && (IsOdf(h,"svfigh")))
							{
								cca7 = h;
							}
							else
							{
								if ((cca8 == BZINVALIDHANDLE) && (IsOdf(h,"svfigh")))
								{
									cca8 = h;
								}
								else
								{
									if ((cca9 == BZINVALIDHANDLE) && (IsOdf(h,"svtank")))
									{
										cca9 = h;
									}
									else
									{
										if ((cca0 == BZINVALIDHANDLE) && (IsOdf(h,"svtank")))
										{
											cca0 = h;
										}
										else
										{
											if ((scav1 == BZINVALIDHANDLE) && (IsOdf(h,"svscav")))
											{
												scav1 = h;
											}
											else
											{
												if ((scav2 == BZINVALIDHANDLE) && (IsOdf(h,"svscav")))
												{
													scav2 = h;
												}
												else
												{
													if ((scav3 == BZINVALIDHANDLE) && (IsOdf(h,"svscav")))
													{
														scav3 = h;
													}
													else
													{
														if ((nsdfgech1 == BZINVALIDHANDLE) && (IsOdf(h,"avwalk")))
														{
															nsdfgech1 = h;
														}
														else
														{
															if ((ccatug == BZINVALIDHANDLE) && (IsOdf(h,"svhaul")))
															{
																ccatug = h;
															}
															else
															{
																if ((avsilo == BZINVALIDHANDLE) && (IsOdf(h,"absilo")))
																{
																	avsilo = h;
																}
															}
...
//		start_convoy_time = Get_Time() + 900.0f;		
		...
		SetObjectiveName(nav1, "Choke Point");
		start_camera1 = true;
		start_done = true;
	}
...
*/
	...
	bool camCancel = false;
	if ((!player_camera_off) && ((next_shot_time < Get_Time()) || ((camCancel=CameraCancelled()))))
	{
		if (camCancel)
		{
...
*/	}
	...
	{
		CameraObject(ccaturret6, 3000, 3000, 3000, ccaturret6);
		if (!cam1_on)
		{
	...
	{
		CameraObject(ccaturret5, 3000, 3000, 3000, ccaturret5);
		if (!cam2_on)
		{
	...
	{
		CameraObject(ccaturret4, 3000, 3000, 3000, ccaturret4);
		if (!cam3_on)
		{
	...
	{
		CameraObject(ccaturret3, 3000, 3000, 3000, ccaturret3);
		if (!cam4_on)
		{
	...
	{
		CameraObject(ccaturret2, 3000, 3000, 3000, ccaturret2);
		if (!cam5_on)
		{
...
// end of camera script for artiliery units ////////////////////
	...
	if	((IsAlive(ccarecycle)) && (turret1_set == false))
	{
		cca1 = BuildObject("svturr", 2, "post1");
		turret1_set = true;
		Defend(cca1);
	}
	if	((IsAlive(ccarecycle)) && (turret2_set == false))
	{
		cca2 = BuildObject("svturr", 2, "post2");
		turret2_set = true;
		Defend(cca2);
	}
	if	((IsAlive(ccarecycle)) && (turret3_set == false))
	{
		cca3 = BuildObject("svturr", 2, "post3");
		turret3_set = true;
		Defend(cca3);
	}
	if	((IsAlive(ccarecycle)) && (turret4_set == false))
	{
		cca4 = BuildObject("svturr", 2, "post4");
		turret4_set = true;
		Defend(cca4);
	}
...
// this is the general warning of the approaching convoy ////////////////////////////////
	...
	if ((!third_warning) && (third_warning_time < Get_Time()))	
	{
		...
		{
			relic = BuildObject("obdata", 0, convoy_geyser);
...
// this is the charon code
	...
	if ((charon_found) && (IsInfo("hbchar") == true) && (!charon_build))
	{
		...
		SetObjectiveName(charon_nav, "Alien Relic");
		charon_build = true;
	}
...
// this is to check and see if the muf is deployed correctly
	if ((objective1) || (third_warning))
	{
		if ((!muf_deployed_good) && (deploy_check < Get_Time()))
		{
			...
			if (IsAlive(nsdfmuf))
			{
				if ((IsDeployed(nsdfmuf)) && (GetDistance(nsdfmuf, convoy_geyser) < 1400.0f))
				{
					if (objective1)
					{
						ClearObjectives();
...
const std::set<std::string>& Misn09Mission::GetPreloadSet()
{
