...
void Misns6Mission::AddObject(BZHandle h)
{
	int closest;
	if (
		(GetTeamNum(h) == 2) &&
...
//		BuildObject("svrecy",1,"spawn1");
		BZHandle cam1 = GetHandle("spcamr0_camerapod");
	...
	for (count=0;count<3;count++)
	{
		...
		if (IsAlive(miners[count]))
		{
			...
			if ((GetLastEnemyShot(miners[count])>0) && (!counter1))
			{
				counter1=true;
				BZHandle a1=BuildObject("bvraz",2,"counter1");
				BZHandle a2=BuildObject("bvraz",2,"counter2");
				Attack(a1,player);
				Attack(a2,player);
			}
	...
	{
		BZHandle temp=BuildObject("bvltnk",2,"counter_attack");
...
const std::set<std::string>& Misns6Mission::GetPreloadSet()
{
