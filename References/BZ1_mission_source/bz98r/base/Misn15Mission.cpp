...
void Misn15Mission::AddObject(BZHandle h)
{
	/*
...
void Misn15Mission::Execute(void)
{
	...
	//	SetAIP("misn15.aip");
		AddScrap(1,10);
		showStuff();
		if (GetHandle("misn15b")!=BZINVALIDHANDLE ) 
		{	
			misn15b=true;
		...
		/*
			BZHandle misl=BuildObject("waspmsl",2,cam1);
			VECTOR_3D from_vec,to_vec;
			from_vec=GetOrigin(misl, );
			to_vec=GetOrigin(player, );
		...
		*/
		SetObjectiveName(cam1, "Geyser Site");
		SetObjectiveName(cam2, "NW Geyser");
		SetObjectiveName(cam3, "NE Geyser");
		SetObjectiveName(cam4, "Geyser Site");
		SetObjectiveName(cam5, "Supply");
		SetObjectiveName(cam6, "Nav Bravo");
		...
		scav_du_jour=recy;
		start_done=true;
		if (cam6!=BZINVALIDHANDLE) SetUserTarget(cam6);
	}
...
const std::set<std::string>& Misn15Mission::GetPreloadSet()
{
