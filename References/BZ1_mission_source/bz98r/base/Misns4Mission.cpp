...
void Misns4Mission::AddObject(BZHandle h)
{
}
...
void Misns4Mission::Execute(void)
{
	...
	*/
	...
	{
		BZHandle h=BuildObject("avfigh",2,"spawn4");
	...
	if (GetTime()>convoy_time)
	{
		...
		BZHandle hauler=BuildObject("svhaul",1,"spawn1");
		convoy_handle[convoy_count]=hauler;
		convoy_count++;
		Goto(hauler,"escort");
	...
	*/
	...
	for (count=0;count<convoy_total;count++)
	{
		if (convoy_handle[count]!=BZINVALIDHANDLE)
		{
...
const std::set<std::string>& Misns4Mission::GetPreloadSet()
{
