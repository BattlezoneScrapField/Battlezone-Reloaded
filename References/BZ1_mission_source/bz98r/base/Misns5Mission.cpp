...
void Misns5Mission::AddObject(BZHandle h)
{
	if (GetTeamNum(h) == 2)
	{
		if (IsOdf(h, "avwalk"))
...
*/
	...
	if ((defender)  && (h1!=BZINVALIDHANDLE) && (!apc_here) &&
	...
	if (GetTime()>apc_wave)
	{
		...
		BZHandle protect=BuildObject("bvtank",2,"spawn7");
...
//			killme=BuildObject("avrecy",2,"spawn7");
			...
			BuildObject("avscav",2,"spawn7");
			BZHandle sam=BuildObject("spcamr",1,"camera1");
...
const std::set<std::string>& Misns5Mission::GetPreloadSet()
{
