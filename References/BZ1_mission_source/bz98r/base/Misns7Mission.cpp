...
static const char * const rescue_msg[3] = { "misns701.wav", "misns702.wav", "misns703.wav" };
...
void Misns7Mission::AddObject(BZHandle h)
{
	if ((avscav1 == BZINVALIDHANDLE) && (IsOdf(h,"bvscav")))
	{
		avscav1 = h;
	}
	else
	{
		if ((avscav2 == BZINVALIDHANDLE) && (IsOdf(h,"bvscav")))
		{
			avscav2 = h;
		}
		else
		{
			if ((avfight1 == BZINVALIDHANDLE) && (IsOdf(h,"bvraz")))		
			{
				avfight1 = h;
			}
			else
			{
				if ((avfight2 == BZINVALIDHANDLE) && (IsOdf(h,"bvraz")))
				{
					avfight2 = h;
				}
				else
				{
					if ((avtank1 == BZINVALIDHANDLE) && (IsOdf(h,"bvtank")))
					{
						avtank1 = h;
					}
					else
					{
						if ((avtank2 == BZINVALIDHANDLE) && (IsOdf(h,"bvtank")))
						{
							avtank2 = h;
						}
						else
						{
							if ((avltnk1 == BZINVALIDHANDLE) && (IsOdf(h,"bvltnk")))
							{
								avltnk1 = h;
							}
							else
							{
								if ((avltnk2 == BZINVALIDHANDLE) && (IsOdf(h,"bvltnk")))
								{
									avltnk2 = h;
								}
								else
								{
									if ((avgech == BZINVALIDHANDLE) && (IsOdf(h,"bvwalk")))
									{
										avgech = h;
									}
									else
									{
										if ((avturr1 == BZINVALIDHANDLE) && (IsOdf(h,"bvturr")))
										{
											avturr1 = h;
										}
										else
										{
											if ((avturr2 == BZINVALIDHANDLE) && (IsOdf(h,"bvturr")))
											{
												avturr2 = h;
											}
											else
											{
												if ((main_tower == BZINVALIDHANDLE) && (IsOdf(h,"abtowe")))
												{
													main_tower = h;
												}
												else
												{
													if ((avtower1 == BZINVALIDHANDLE) && (IsOdf(h,"abtowe")))
													{
														avtower1 = h;
													}
													else
													{
														if ((avtower2 == BZINVALIDHANDLE) && (IsOdf(h,"abtowe")))
														{
															avtower2 = h;
														}
														else
														{
															if ((main_power == BZINVALIDHANDLE) && (IsOdf(h,"abwpow")))
															{
																main_power = h;
															}
															else
															{
																if ((avpower1 == BZINVALIDHANDLE) && (IsOdf(h,"abwpow")))
																{
																	avpower1 = h;
																}
																else
																{
																	if ((avpower2 == BZINVALIDHANDLE) && (IsOdf(h,"abwpow")))
																	{
																		avpower2 = h;
																	}
																	else
																	{
																		if ((IsAlive(svmuf)) && (newmuf == BZINVALIDHANDLE) && (IsOdf(h,"svmuf")))
																		{
																			newmuf = h;
																		}
																		else
...
//	}
	...
	if (!start_done)
	{
        ...
        GameObject *t = GameObjectHandle::GetObj(boxes);
        t->SetMaxHealth(0);
	}
...
// this is what happens if the player attacks the american scavengers right away
	...
	if (!jail_dead)
	{
		if ((((IsAlive(avscav1)) && (GetHealth(avscav1)<0.91f))
			||
			((IsAlive(avscav2)) && (GetHealth(avscav2)<0.91f))
			||
			((IsAlive(main_power)) && (GetHealth(main_power)<0.95f))
			||
			((IsAlive(avrecycle)) && (GetHealth(avrecycle)<0.95f))
			||
			((IsAlive(avrig)) && (GetHealth(avrig)<0.95f)))	&& (!nsdf_adjust))
		{
			nsdf_adjust = true;
		}
	}

	if ((IsAlive(jail)) && (!in_base) && (GetHealth(jail) < 0.50f))
	{ 
		in_base = true;
	}
...
// this is the apc telling the player to get him out of there
	...
	if ((jail_camera_off) && (IsAlive(apc)) && (avfigh2_time < Get_Time()) && (!fully_loaded)
		&& (GetHealth(apc)<0.80f) && (!apc_panic_message))
	{
...
/*	if ((avmuf_built) && (IsAlive(avmuf)) && (GetHealth(avmuf)<0.50f) 
...
//	}
	...
	if ((jail_unit_spawn) && (GetDistance(con1, apc) < 20.0f) && (!con1_dead) && (!con1_in_apc))
	{
		...
		if ((con1_in_apc) && (con1_pickup_time < Get_Time()) && (!con1_safe))
		{
			RemoveObject(con1);
			AddPilot(1, 1);
			AudioMessage(rescue_msg[con1_safe + con2_safe + con3_safe]); // engineer aboard message based on safe count 726
...
//			supply_message_time = Get_Time() + 3.0f;
		...
		if ((supply2_message) && (supply_spawn_time < Get_Time()) && (!supplies_spawned))
		{
...
const std::set<std::string>& Misns7Mission::GetPreloadSet()
{
