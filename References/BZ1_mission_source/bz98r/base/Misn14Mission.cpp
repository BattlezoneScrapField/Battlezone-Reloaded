	...
	union {
		struct {
			BZHandle
				...
				guy1,guy2,guy3,tow1,tow2,tow3,tow4,pow1,pow2,
				h_last;
		};
		BZHandle h_array[22];
	};
...
void Misn14Mission::AddObject(BZHandle h)
{
	if (
		(IsOdf(h, "avapc")) &&
		(GetTeamNum(h) == 1) 
...
void Misn14Mission::Execute(void)
{
	...
	if (!start_done)
	{
		...
		pow1=GetHandle("sblpow1_powerplant");
		pow2=GetHandle("sblpow55_powerplant");
		SetObjectiveName(cam1, "Foothill Geysers");
		SetObjectiveName(cam2, "Canyon Geysers");
		SetObjectiveName(cam3, "CCA Base");
		SetObjectiveName(cam4, "Plateau Geysers");
		start_done=true;
	...
	*/	
	if (GetTime()>beacon_time1)
	{
		...
		SetIndependence(guy1, 0);
		SetIndependence(guy2, 0);
		SetIndependence(guy3, 0);
		Defend(guy1);
		Defend(guy2);
		Defend(guy3);
	...
	if (
		(!lost) &&
		(beacon1!=BZINVALIDHANDLE) && (!rescue1) &&
	...
	if ((beacon1!=BZINVALIDHANDLE) && (apc!=BZINVALIDHANDLE) && (!rescue1)
	...
	if ((beacon1!=BZINVALIDHANDLE) && (rescue1)
	...
	if (
		(!lost) &&
		(beacon2!=BZINVALIDHANDLE) && (!rescue2) &&
		((!IsAlive(guy1)) || (!IsAlive(guy2)) || (!IsAlive(guy3)))
		)
	{
		lost=true;
	...
	if ((beacon2!=BZINVALIDHANDLE) && (apc!=BZINVALIDHANDLE) && (!rescue2)
	...
	if ((beacon2!=BZINVALIDHANDLE) && (rescue2)
	...
	if (GetTime()>beacon_time3)
	{
		...
		if (
			(!lost) &&
		(beacon3!=BZINVALIDHANDLE) && (!rescue3) &&
		((!IsAlive(guy1)) || (!IsAlive(guy2)) || (!IsAlive(guy3)))
		)
	{
		lost=true;
	...
	if ((beacon3!=BZINVALIDHANDLE) && (apc!=BZINVALIDHANDLE) && (!rescue3)
	...
	if ((beacon3!=BZINVALIDHANDLE) && (rescue3)
		&& (rescue_finish3<GetTime()))
	{
		...
		*/
		...
		rescue_finish3=99999.0f;
		ClearObjectives();
		AddObjective("misn1401.otf",GREEN);
	}
	/*
	...
	*/
	...
	if ((wave_count>3) && (!gen_message) && (rescue3))
	{
		...
		gen_message=true;
		BZHandle foe= GetNearestEnemy(player);
...
const std::set<std::string>& Misn14Mission::GetPreloadSet()
{
