...
void Misn08Mission::AddObject(BZHandle h)
{
	if ((nsdfmuf == BZINVALIDHANDLE) && (IsOdf(h,"avmu8")))
	{
		nsdfmuf = h;
	}
	else
	{
		if ((ccaapc == BZINVALIDHANDLE) && (IsOdf(h,"svapc")))
		{
			ccaapc = h;
		}
		else
		{
			if ((guntower1 == BZINVALIDHANDLE) && (IsOdf(h,"abtowe")))
			{
				guntower1 = h;
			}
			else
			{
				if ((guntower2 == BZINVALIDHANDLE) && (IsOdf(h,"abtowe")))
				{
					guntower2 = h;
				}
			}
...
//		SetPilot(1, 30);
		...
		SetObjectiveName(nav5, "Colorado Base");
		SetObjectiveName(nav4, "CCA Main Base");
...
// colorado gets attacked by gech //////////////////////////////////////////////////////////////////////////
	...
	if ((colorado_under_attack) && IsAlive(ccagech3))
	{
		SetCurAmmo(ccagech3, GetMaxAmmo(ccagech3));	// keep ammunition topped off so it doesn't run out
	}
	...
	if ((colorado_message4_time < Get_Time()) && (!kill_colorado) && !IsAudioMessagePlaying())
	{
		kill_colorado = true;
	}
	...
	if ((IsAlive(colorado)) && (!kill_colorado))
	{
		...
		{
			SetCurHealth(colorado, GetMaxHealth(colorado)); //prevent colorado from dying too early
	...
	if ((colorado_message3) && (kill_colorado) && (!colorado_message4))
	{
		...
		remove_nav5_time = Get_Time() + 3.0f;
...
// this is what happens when the player encounters the gech before it reaches the nav point ////////////////////
	...
	if ((gech_found) && (no_stumble_check < Get_Time()) && (!run_into_other_gech) && (!second_gech_warning))
	{
		...
		if ((gech_found1) && (GetDistance(gech_trigger2, ccagech2) < 100.0f) && (!second_gech_warning))
		{
			...
			SetObjectiveName(nav2, "Nav Alpha 1");
		...
		if ((gech_found2) && (GetDistance(gech_trigger3,ccagech1) < 100.0f) && (!second_gech_warning))
		{
			...
			SetObjectiveName(nav3, "Nav Alpha 2");
			second_gech_warning = true;
		}
	}
...
// this will make the gechs use the popper
	...
	if ((IsAlive(ccagech1)) && (!gech1_blossom) && (GetHealth(ccagech1) < 0.25f))
	{
	...
	if ((IsAlive(ccagech2)) && (!gech2_blossom) && (GetHealth(ccagech2) < 0.25f))
	{
	...
	if ((IsAlive(ccagech3)) && (!gech3_blossom) && (GetHealth(ccagech3) < 0.25f))
	{
...
// (there's a one-frame delay between calling AudioMessage and IsAudioMessagePlaying returning true)
	...
	if (!won_mission && cerb_found && base_dead && !IsAudioMessagePlaying())
	{
		SucceedMission(Get_Time() + 3.0f, "misn08w1.des"); //well done
		won_mission = true;
	}

	// check if recycler destroyed
	if (!lost_mission && game_over && !IsAudioMessagePlaying())
	{
		FailMission(Get_Time() + 3.0f, "misn08f1.des"); //lost recycler
		lost_mission = true;
	}
...
const std::set<std::string>& Misn08Mission::GetPreloadSet()
{
