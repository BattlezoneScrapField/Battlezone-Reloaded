...
bool Misn07Mission::Load(file fp)
{
	...
	for (int i = 0; i < p_count; i++)
		ret = ret && in_ptr(fp, (void **)&p_array[i], sizeof(p_array[0]), "Misn07Mission::p_array", this);
...
void Misn07Mission::AddObject(BZHandle h)
{
}
...
*/
	...
	{ 
		...
		SetObjectiveName(nav3, "Rendezvous Point");
...
//		&& (GetHealth(svpatrol2_1)<0.98f) // (GetLastEnemyShot(svpatrol2_1)<0)
//		&& (GetHealth(svpatrol2_2)<0.98f)) //(GetLastEnemyShot(svpatrol2_2)<0)) 
	{
...
{																							
	if ((rendezous_check < Get_Time()) && (!rendezvous) && (!alarm_on))
	{
		...
		{																					
			...
			{
				MAT_3D M = GetTransform(wingtank2);
				RemoveObject(wingtank2);
				new_tank1 = BuildObject("avtank", 1, M);
			}
			if (IsAlive(wingtank3))
			{
				MAT_3D M = GetTransform(wingtank3);
				RemoveObject(wingtank3);
				new_tank2 = BuildObject("avtank", 1, M);
			}
			ClearObjectives();																
			...
			SetObjectiveName(nav1, "CCA Outpost");
	...
	{
		if (svpatrol2_1!=BZINVALIDHANDLE)
		{
			Attack(svpatrol2_1, user);
		}
		if (svpatrol2_2!=BZINVALIDHANDLE)
		{
...
//			AudioMessage("misn0702.wav"); // Rookie "I found an overlook"										
	...
	{
		SetObjectiveName(jump_cam, "Volcano Peak");
	}
...
//			recon_message2_time = Get_Time() + 480.0f;
	...
	if ((jump_cam_spawned) && (jump_cam!=BZINVALIDHANDLE))
	{
		SetObjectiveName(jump_cam, "Volcano Peak");
	}
...
if (!first_objective)
{
	if (alarm_on)
	{
		...
		if ((ccacomtower!=BZINVALIDHANDLE) && (GetDistance(user, ccacomtower) < 170.0f) && (!alarm_sound))
		{
		...
		if ((start_evac) && (unit_spawn_time < Get_Time()) && (!unit_spawn) && (!alarm_special))// spawns cca soldiers and tells them to go to their tanks
		{
			...
			if (parkturret1 != user) {
				MAT_3D M = GetTransform(parkturret1);
				RemoveObject(parkturret1);
				spawn_turret1 = BuildObject("svturr", 2, M);
				Defend(spawn_turret1);
			}
			if (parkturret2 != user) {
				MAT_3D M = GetTransform(parkturret2);
				RemoveObject(parkturret2);
				spawn_turret2 = BuildObject("svturr", 2, M);
				Defend(spawn_turret2);
			}
			// these lines tell the soviet pilots to get to their ships
			if (parked1!=BZINVALIDHANDLE)
			{
				Retreat(pilot1, parked1, 1);
			}
			
			if (parked2!=BZINVALIDHANDLE)
			{
				Retreat(pilot2, parked2, 1);
			}

			if (parked3!=BZINVALIDHANDLE)
			{
		...
		{
			if ((!IsAlive(pilot1)) && (parked1!=BZINVALIDHANDLE))
			{
				Attack(parked1, user);
			}
			if ((!IsAlive(pilot2)) && (parked2!=BZINVALIDHANDLE))
			{
				Attack(parked2, user);
			}
			if ((!IsAlive(pilot3)) && (parked3!=BZINVALIDHANDLE))
			{
				Attack(parked3, user);
			}
...
*/	
		...
		if ((unit_spawn) && (alarm_special))
		{
			...
			if ((!IsAlive(pilot4)) && (parkturret1!=BZINVALIDHANDLE))
			{
				Retreat(parkturret1, "turret1_spot");
			}
		}
...
//			((!alarm_special) && (!IsAlive(ccaguntower2)) && (!forces_enroute)))
		if ((IsAlive(ccacomtower)) && (GetHealth(ccacomtower)<0.50f) && (!forces_enroute))
		{
...
//		alarm_time = Get_Time() + 20.0f;
	...
	{
		if (
			IsDamaged(ccaguntower1) ||
			IsDamaged(ccaguntower2) ||
			IsDamaged(ccacomtower) ||
			IsDamaged(powrplnt1) ||
			// IsDamaged(powrplnt2) ||
			IsDamaged(barrack1) ||
			IsDamaged(barrack2) ||
			...
			IsDamaged(parked2) ||
			IsDamaged(parked3) ||
			IsDamaged(parkturret1) ||
			IsDamaged(parkturret2)
			)
		{
			trigger1 = true;
		}
...
//			SetObjectiveOff(svpatrol3_2);
	...
	// this is what happens if an enemy unit gets away - tanks will come out
		...
		if ((patrola1) && (!retreat_success) && (!alarm_on)
			...
			&& (ccarecycle!=BZINVALIDHANDLE) && (IsAlive(ccarecycle)))
		{
			SetObjectiveOff(svpatrol1_1);		
...
/*
	...
	{
		SetObjectiveName(nav4, "Testing Range");
	}
...
// this is how the rookie tells the player he's under attack
	...
	{
		SetObjectiveName(nav5, "Mine Field");
	}
	...
	if ((rookie_lost) && (reach_mine_time < Get_Time()) && (!mine_pathed))
	{
		...
		{
			SetObjectiveName(becon1, "Mine Path 1");
		}
		if ((mine_pathed) && (becon2!=NULL))
		{
			SetObjectiveName(becon2, "Mine Path 2");
		}
		if ((mine_pathed) && (becon3!=NULL))
		{
			SetObjectiveName(becon3, "Mine Path 3");
		}
		if ((mine_pathed) && (becon4!=NULL))
		{
			SetObjectiveName(becon4, "Mine Path 4");
		}
...
*/
	if ((first_objective) && (!next_mission) && (next_mission_time < Get_Time()))
	{
		...
		SetObjectiveName(nav6, "Utah Rendezvous");
		SetObjectiveName(nav7, "CCA BASE");
		AddScrap(1, 30);
		SetPilot(1, 20);
		AddScrap(2, 60);
		SetPilot(2, 40);
...
// win/loose conditions ///////////////////
	...
	if ( (!IsAlive(user)) && (IsOdf(user, "asuser")) )
	{
		FailMission(Get_Time() + 5.0f);
	}
...
const std::set<std::string>& Misn07Mission::GetPreloadSet()
{
