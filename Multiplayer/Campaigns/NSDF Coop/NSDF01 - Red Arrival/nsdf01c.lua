...
-- For coop mission authors, I'd recommend familiarizing yourself with a few useful parts of my library.
-- The net module contains many helpers for synchronization as well as signaling between host and remotes.
-- The base mission class is used for singleplayer missions and also contains many methods that are also used
-- by the coop mission class. You don't need to dive into the implementation details, but it's good to get an
-- overview of the different methods. The time module has a multi purpose timer class that is used frequently
-- in different parts of the library as well as this mission. The util module also has functions to defer
-- function calls until the next update or until a certain time period has elapsed.
...
-- 4 player coop
local my_team = vsp.team.make_team("NSDF", 1, 2, 3, 4)
...
local mission = vsp.coop_mission.make_coop(my_team)
